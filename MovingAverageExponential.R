...
?movavg()
n_variable <- 7
trend_EUR <- data.frame("trend_Euro"=movavg(EuroForecast.df$Today_EUR_USDClose, n=n_variable, type = "e"))
length(trend_EUR$trend_Euro)
trend_EUR<-data.frame("trend_Euro"=trend_EUR[1:(length(trend_EUR$trend_Euro)-1),])
length(trend_EUR$trend_Euro)
na_valid_6 <- data.frame("trend_Euro"=matrix(NA, nrow = 1, ncol = 1)) #have to be previous rolling average
trend_EURO_2 <- rbind(na_valid_6,trend_EUR)


trend_SP <- data.frame("trend_SP"=movavg(EuroForecast.df$Prev_S.P_500_Close, n=n_variable,type = "e"))
na_valid_6 <- data.frame("trend_SP"=matrix(NA, nrow = 0, ncol = 1))
trend_SP_2 <- rbind(na_valid_6,trend_SP)
head(trend_SP, n=13)

trend_Oil <- data.frame("trend_Oil"=movavg(EuroForecast.df$Prev_Brent_Crude_Spot, n=n_variable, type = "e"))
na_valid_6 <- data.frame("trend_Oil"=matrix(NA, nrow = 0, ncol = 1))
trend_Oil_2 <- rbind(na_valid_6,trend_Oil)

trend_EuroBasket<- data.frame("trend_EuroBasket"=movavg(EuroForecast.df$Prev_European_Basket_Close, n=n_variable, type = "e"))
na_valid_6 <- data.frame("trend_EuroBasket"=matrix(NA, nrow = 0, ncol = 1))
trend_EuroBasket_2 <- rbind(na_valid_6,trend_EuroBasket)
str(trend_EuroBasket_2)

trend_Com<- data.frame("trend_Com"=movavg(EuroForecast.df$Prev_Commodities.Basket.Close, n=n_variable, type = "e"))
na_valid_6 <- data.frame("trend_Com"=matrix(NA, nrow = 0, ncol = 1))
...
everything.plot <- ggplot(plot_all, aes(x=TimeSeries)) +
  ...
  labs(title="Exponential Moving Average Multi-Variable Training & Validation Predictions with Residuals")
everything.plot
